...
def generate_scene_graph_from_store_dict(args):
    ...
    for episode in dataset.episodes:
        ...
        set_articulated_agent_base_state(sim, graph_positions[0], graph_orientations[0], agent_id=0)
        ...
        set_articulated_agent_base_state(sim, graph_positions[1], graph_orientations[1], agent_id=0)
        observations = env.step({"action": (), "action_args": {}})
        env._episode_over = False
...
def generate_scene_graph_for_single_task(args):
    ...
    for episode in dataset.episodes:
        ...
        if not check_if_sample:
            continue
        episode_json_path = os.path.join(episode_dir,'data','data_trans.json')
        ...
        for frame_info in episode_info:
            ...
            set_articulated_agent_base_state(sim, pos_info, orien_info, agent_id=0)
            observations = env.step({"action": (), "action_args": {}})
            env._episode_over = False
            for obs_key in obs_keys:
                if obs_key in observations:
...
def generate_episode_image_from_store_dict(args):
    ...
    for episode in dataset.episodes:
        ...
        with open(episode_json_path,'r') as file:
            episode_json = json.load(file)      #用data_trans.json
        get_obj_json_path = os.path.join(episode_dir,f'episode_{episode_id}.json')
        ...
        for frame_info in episode_info:
            ...
            for obs_key in obs_keys:
                if obs_key in observations:
                    image = observations[obs_key]
                    image_file_name = f"{frame_num}_agent_0_head_rgbFetchRobot_head_rgb.png"
                    image_file_path = os.path.join(episode_dir, image_file_name)
                    save_image(image, image_file_path)
                # print("key:",key)
            print("observations['arm_workspace_points']:",observations['arm_workspace_points'])
            try:
                metadata.append({
                    "episode_id": episode.episode_id,
                    "step": frame_num,
