...
import pdb,cv2
def overlay_semantic_on_rgb(semantic_image, rgb_image):
    if len(semantic_image.shape) == 3 and semantic_image.shape[2] == 1:
        semantic_image = semantic_image[:, :, 0]
    # 将语义图像转换为伪彩色
    color_map = cv2.applyColorMap(semantic_image.astype(np.uint8), cv2.COLORMAP_BONE)
    # 将语义信息覆盖在 RGB 图像上
    overlayed_image = cv2.addWeighted(rgb_image, 0.5, color_map, 0.5, 0)
...
def get_detected_objects(observations, *args, **kwargs):
...
def main(args):
    ...
    for episode in dataset.episodes:
        ...
        for idx, (position, orientation) in enumerate(zip(graph_positions, graph_orientations)):
            ...
            # print("semantic_observations:",observations['head_semantic'].shape)
            # print("rgb_observations:",observations['head_rgb'].shape)

            obs_file_list = []
            for obs_key in args.obs_keys:
                if obs_key in observations:
                    image = observations[obs_key]
                    # if obs_key == 'head_semantic':  # 语义图像
                    #     semantic_image = image
                    #     count_of_3 = np.sum(semantic_image == 3)
                    #     # print("equal3:",count_of_3)
                    #     # print(f"{get_detected_objects(observations)}")
                    # elif obs_key == 'head_rgb':  # RGB 图像
                    #     rgb_image = image
                    observation_keys = list(observations.keys())
                    # print("observations_keys:",observation_keys,flush = True)
                    # if 'semantic_image' in locals() and 'rgb_image' in locals():
                    #     # 叠加语义信息
                    #     overlayed_image = overlay_semantic_on_rgb(semantic_image, rgb_image)
                       
                    #     image_file_name = f"episode_{episode.episode_id}_{obs_key}_{idx}_overlay.png"
                    #     image_file_path = os.path.join(episode_dir, image_file_name)
                    #     cv2.imwrite(image_file_path, overlayed_image)
...
def parse_args():
    ...
    parser.add_argument("--dist_to_target", type=float, default=4.0)
