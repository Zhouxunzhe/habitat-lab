...
from habitat.tasks.rearrange.articulated_agent_manager import (
    ArticulatedAgentManager, ArticulatedAgentData
)
import magnum as mn
import numpy as np
...
class HabitatSimV1(HabitatSim):
    ...
    def __init__(self, config: DictConfig) -> None:
        ...
        self.ep_info: Optional[Episode] = None 
        ...
        self.navmesh_visualization = True
    ...
    @add_perf_timing_func()
    def reconfigure(self, habitat_config: DictConfig, ep_info: Episode, should_close_on_new_scene: bool = True) -> None:
        self.ep_info = ep_info
        new_scene = self.prev_scene_id != ep_info.scene_id
        self.agents_mgr.pre_obj_clear()

        t_start = time.time()
        super().reconfigure(habitat_config, ep_info, should_close_on_new_scene)
    ...
    def _load_navmesh(self, ep_info):
        ...
        navmesh_path = osp.join(base_dir, scene_name, scene_name + ".navmesh")
