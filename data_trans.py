...
def _3d_to_2d(matrix, point_3d):
        ...
        project_mar = projection_matrix.T
        print("project_mar:",project_mar)
        # print("mar:",np.append(point_3d,1),np.array(matrix).reshape(4, 4))s
        point_transform = np.dot(np.append(point_3d,1),np.array(matrix).T)
        projected_point_3d = np.dot(point_transform,project_mar)
...
data_path = './video_dir/image_dir/episode_118'
data_trans = []
skip_len = 20
...
for i in range(0,len(data['entities'])-skip_len):
    ...
    print("agent_0_cam:",np.array(agent_0_cam))
    ...
    agent_1_prepix = _3d_to_2d(matrix = agent_1_cam,point_3d = agent_1_objpos)
    ...
    result = {
        ...
        "agent_1_camera_point":agent_1_prepix.tolist()
